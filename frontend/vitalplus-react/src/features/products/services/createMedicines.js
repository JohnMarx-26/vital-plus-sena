...
export const createMedicamento = async (formData) => {
  // Construimos el FormData para enviar imagen + datos
  ...
  if (formData.administrationRoute) {
    data.append(
      ...
      parseInt(formData.administrationRoute)
    );
  }
  ...
  // data.append("precio_descuento", parseFloat(data.salePriceDiscount || 0).toFixed(2));
  data.append(
    "precio_descuento",
    parseFloat(formData.salePriceDiscount || 0).toFixed(2)
  );
  ...
  const response = await fetch(`${API_URL}/medicamentos/`, {
    method: "POST",
    body: data,
    // no se pone el  Content-Type, el navegador lo asigna automáticamente con el boundary del FormData
  });
